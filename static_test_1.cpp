...
int main() {
    ...
    VideoCapture cap("/Users/chenxuan/Downloads/OpenCV_Proj/static2.mov");
    if (!cap.isOpened()) {
    ...
    while (cap.read(srcFrame)) {
        ...
        for (int i = 0; i <= 3; i++) {
            ...
            if (i == 0)
                cv::putText(srcFrame, "1", Point(pt.x, pt.y), FONT_HERSHEY_SIMPLEX, 1.0, Scalar(0, 255, 0), 3);
            else if (i == 1)
                cv::putText(srcFrame, "2", Point(pt.x, pt.y), FONT_HERSHEY_SIMPLEX, 1.0, Scalar(0, 255, 0), 3);
            else if (i == 2)
                cv::putText(srcFrame, "3", Point(pt.x, pt.y), FONT_HERSHEY_SIMPLEX, 1.0, Scalar(0, 255, 0), 3);
            else if (i == 3)
                cv::putText(srcFrame, "4", Point(pt.x, pt.y), FONT_HERSHEY_SIMPLEX, 1.0, Scalar(0, 255, 0), 3);
            circle_cur[i].x = pt.x;
            circle_cur[i].y = pt.y;
        ...
        if (count == 1) {
            homoMat = findHomography(realplane, imgplane, RANSAC, 5.0);     // RANSAC
            invertMat = homoMat.inv();
            cout << homoMat << endl << endl;
        }
        // homoMat = findHomography(realplane, imgplane);   // Normal
        // Mat invertMat = homoMat.inv();
        Mat centerVect = (Mat_<double>(3, 1) << center_cur.x, center_cur.y, 1);
        Mat centerVectTarget_cur =  invertMat * centerVect;
        ...
        centerVectTarget_cur.copyTo(centerVectTarget_pre);
